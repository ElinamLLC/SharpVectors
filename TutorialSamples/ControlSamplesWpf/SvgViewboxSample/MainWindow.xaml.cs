...
namespace SvgViewboxSample
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void OnWindowLoaded(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                if (requestInfo != null)
                {
                    using (HttpWebResponse response = (HttpWebResponse)requestInfo.GetResponse())
                    {
                        ...
                        webSvgViewbox.StreamSource = webStream;
                    }
                }
            }
            catch (Exception ex)
            ...
            try
            {
                ...
                if (resourceInfo != null)
                {
                    ...
                    {
                        resourceSvgViewbox.StreamSource = resourceStream;
                    }
                }
            }
            catch (Exception ex)
        ...
        private void OnLoadMethodChanged(object sender, RoutedEventArgs e)
        {
            ...
            loadSvgViewbox.Unload(true); // Will display message "Loading..."
            //loadSvgViewbox.Unload(true, "Loading, please wait..."); // Will display message "Loading, please wait"
            ...
            try
            {
                if (rbResource.IsChecked != null && rbResource.IsChecked.Value)
                {
                    ...
                    if (rbUri.IsChecked != null && rbUri.IsChecked.Value)
                    {
                        loadSvgViewbox.Load(resourceUri, useAsync);
                    }
                    else
                    {
                        ...
                        using (var resourceStream = resourceInfo.Stream)
                        {
                            ...
                            {
                                loadSvgViewbox.Load(resourceStream, useCopyStream, useAsync);
                            }
                            else // otherwise, we send a cloned content of the stream...
                            {
                                ...
                                loadSvgViewbox.Load(contentStream, useCopyStream, useAsync);
                            }
                        }
                    }
                }
                else
                {
                    ...
                    if (rbUri.IsChecked != null && rbUri.IsChecked.Value)
                    {
                        ...
                        loadSvgViewbox.Load(webUri, useAsync);
                    }
                    else
                    {
                        ...
                        using (HttpWebResponse webResponse = (HttpWebResponse)webRequest.GetResponse())
                        {
                            ...
                            {
                                loadSvgViewbox.Load(responseStream, useCopyStream, useAsync);
                            }
                            else // otherwise, we send a cloned content of the stream...
                            {
                                MemoryStream contentStream = new MemoryStream();
        ...
        private async void OnLoadAsyncMethodChanged(object sender, RoutedEventArgs e)
        {
            ...
            //loadSvgViewboxAsync.Unload(true); // Will display message "Loading..."
            loadSvgViewboxAsync.Unload(true, messageText); // Will display customized message
            ...
            try
            {
                if (rbResourceAsync.IsChecked != null && rbResourceAsync.IsChecked.Value)
                {
                    ...
                    if (rbUriAsync.IsChecked != null && rbUriAsync.IsChecked.Value)
                    {
                        await loadSvgViewboxAsync.LoadAsync(resourceUri);
                    }
                    else
                    {
                        var resourceInfo = Application.GetResourceStream(resourceUri);
                        ...
                        using (var resourceStream = resourceInfo.Stream)
                        {
                            if (useCopyStream) // if true, let the control copy/clone the stream...
                            {
                                await loadSvgViewboxAsync.LoadAsync(resourceStream);
                            }
                            else // otherwise, we send a cloned content of the stream...
                            {
                                MemoryStream contentStream = new MemoryStream();
                                ...
                                await loadSvgViewboxAsync.LoadAsync(contentStream);
                            }
                        }
                ...
                else
                {
                    ...
                    {
                        Uri webUri = new Uri(webPath);
                        await loadSvgViewboxAsync.LoadAsync(webUri);
                    }
                    else
                    {
                        HttpWebRequest webRequest = (HttpWebRequest)WebRequest.Create(webPath);
                        ...
                        using (HttpWebResponse webResponse = (HttpWebResponse)webRequest.GetResponse())
                        {
                            ...
                            if (useCopyStream) // if true, let the control copy/clone the stream...
                            {
                                await loadSvgViewboxAsync.LoadAsync(responseStream, useCopyStream);
                            }
                            else // otherwise, we send a cloned content of the stream...
                            {
                                MemoryStream contentStream = new MemoryStream();
                                ...
                                await loadSvgViewboxAsync.LoadAsync(contentStream, useCopyStream);
                            }
                        }
        ...
        private void ReportError(string message)
        {
            ...
            MessageBox.Show(message, "SharpVector: SvgViewbox Sample - C#", MessageBoxButton.OK, MessageBoxImage.Error);
        }
