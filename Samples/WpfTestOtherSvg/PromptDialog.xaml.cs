    ...
    public partial class PromptDialog : Window
    {
        ...
        private void OnDownloadClicked(object sender, RoutedEventArgs e)
        {
            ...
            if (isDownloadTests)
            {
                ...
                {
                    client.DownloadFileCompleted += delegate (object other, AsyncCompletedEventArgs eventArgs)
                    {
                        bool result = !eventArgs.Cancelled;
                        if (!result || !File.Exists(downloadedTests))
                        {
                            _cancelledOrError = true;
                        ...
                        new Thread(delegate() {
                            using (ZipFile zip = ZipFile.Read(downloadedTests))
                            {
                                ...
                                string extractedDir = Path.Combine(workingDir, "resvg-test-suite-master");
                                if (!Directory.Exists(extractedDir))
                                {
                                    Directory.Delete(extractedDir, true);
                                }

                                Directory.Move(Path.Combine(extractedDir, "fonts"), Path.Combine(outputDirTests, "fonts"));
                                Directory.Move(Path.Combine(extractedDir, "images"), Path.Combine(outputDirTests, "images"));
                                Directory.Move(Path.Combine(extractedDir, "png"), Path.Combine(outputDirTests, "png"));
                                Directory.Move(Path.Combine(extractedDir, "svg"), Path.Combine(outputDirTests, "svg0"));
                    ...
                    client.DownloadFileAsync(new Uri(urlTests), downloadedTests);
                }
            }
            ...
            if (isDownloadBatik)
            {
                using (WebClient client = new WebClient())
                {
                    ...
                    {
                        bool result = !eventArgs.Cancelled;
                        if (!result || !File.Exists(downloadedBatik))
                        {
                            _cancelledOrError = true;
                        ...
                        new Thread(delegate () {
                            using (ZipFile zip = ZipFile.Read(downloadedBatik))
                            {
                    ...
                    client.DownloadFileAsync(new Uri(urlBatik), downloadedBatik);
                }
            }
            ...
            if (isDownloadResvg)
            {
                using (WebClient client = new WebClient())
                {
                    ...
                    {
                        bool result = !eventArgs.Cancelled;
                        if (!result || !File.Exists(downloadedResvg))
                        {
                            _cancelledOrError = true;
                        ...
                        new Thread(delegate() {
                            using (ZipFile zip = ZipFile.Read(downloadedResvg))
                            {
                                ...
                                zip.ExtractAll(extractedDirResvg);
                            }
                    ...
                    client.DownloadFileAsync(new Uri(urlResvg), downloadedResvg);
                }
            }
        }
#else
