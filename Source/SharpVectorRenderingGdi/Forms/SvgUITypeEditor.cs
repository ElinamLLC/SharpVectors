...
{
    internal sealed class SvgFilesUITypeEditor : UITypeEditor, IDisposable
    {
        ...
        public override object EditValue(ITypeDescriptorContext context, IServiceProvider provider, object value)
        {
            if ((context != null) && (provider != null))
            {
                ...
                {
                    if (this._fileDlg == null)
                    {
                        this._fileDlg = new OpenFileDialog();
                    }
                    _fileDlg.Multiselect = false;
                    _fileDlg.Filter      = "Svg Files (*.svg;*.svgz;*.xml)|*.svg;*.svgz;*.xml|All files (*.*)|*.*";
                    _fileDlg.FileName    = "";
                    if (_fileDlg.ShowDialog() == DialogResult.OK)
                    {
                        value = _fileDlg.FileName;
                    }
                }
            }
            return value;
